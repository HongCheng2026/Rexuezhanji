  ...
  const pickupWeapons = {
    laser: {
      ...
      damageMultiplierAt(level) {
        return pickLevelValue([1, 1.14, 1.3, 1.48, 1.68, 1.9, 2.12, 2.35, 2.6, 2.88], level);
      },
      fireIntervalAt(level) {
        return 1 - (clampLevel(level) - 1) * 0.055;
      }
    },
    spread: {
      id: "spread",
      ...
      maxLevel: MAX_WEAPON_LEVEL,
      damageMultiplierAt(level) {
        return pickLevelValue([1, 1.1, 1.2, 1.32, 1.45, 1.58, 1.72, 1.86, 2.02, 2.2], level);
      },
      coneAngleAt(level) {
        return 30 + (clampLevel(level) - 1) * 10;
      }
    },
    missile: {
      id: "missile",
      ...
      maxLevel: MAX_WEAPON_LEVEL,
      damageMultiplierAt(level) {
        return pickLevelValue([1, 1.12, 1.25, 1.4, 1.56, 1.74, 1.93, 2.14, 2.36, 2.6], level);
      },
      targetCountAt(level) {
        return Math.min(8, Math.max(1, Math.ceil(clampLevel(level) * 0.8)));
      }
    }
  };
