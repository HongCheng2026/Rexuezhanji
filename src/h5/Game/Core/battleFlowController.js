(function registerBattleFlowController(root) {
  ...
  function beginAuthorizedBattle(level, ticket) {
    ...
    currentLoadout = assignCurrentLoadout((shared.fighterUpgradeApi || shared.combatStats) && (shared.fighterUpgradeApi || shared.combatStats).generateBattleLoadout
  ...
  function refundBattleEnergy() {
    ...
    profile.resources.energy = Math.max(0, Math.floor(profile.resources.energy || 0)) + amount;
  ...
  function abortBattle(target) {
    ...
    return {
      ...
      refundBattleEnergy: refundBattleEnergy
    };
  }
