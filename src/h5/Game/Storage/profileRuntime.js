(function registerProfileRuntime(root) {
  ...
  // 本地存档地址：
  // 默认 rxgame_save_v5 —— 你的原存档，游戏照常读取，完全不动。
  // 独立测试档：想测试时，打开游戏链接后加 ?slot=测试名，
  // 例如 ?slot=test -> 全新独立档 rxgame_save_test，与原档互不干扰、各存各的。
  var slotMatch = (root.location && root.location.search || "").match(/[?&]slot=([^&]+)/);
  var STORAGE_KEY = slotMatch ? "rxgame_save_" + decodeURIComponent(slotMatch[1]) : "rxgame_save_v5";
  ...
  function unlockForLocalTest(profile) {
    ...
    if (testFlags.LOCAL_TEST_UNLOCK_ALL_LEVELS) {
      var levelCount = (levelsConfig.levels || []).length || 3;
      profile.unlockedLevel = levelCount;
    }
    if (testFlags.LOCAL_TEST_UNLOCK_ALL_PILOTS) {
    ...
    if (testFlags.LOCAL_TEST_UNLOCK_ALL_SHIPS) {
      profile.owned = profile.owned || {};
