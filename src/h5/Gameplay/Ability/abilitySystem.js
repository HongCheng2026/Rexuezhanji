(function registerAbilitySystem(root) {
  ...
  function getWeaponModifiers(state, loadout, weaponType) {
    var active = scope.activeSkillSystem && scope.activeSkillSystem.getWeaponModifiers
      ? scope.activeSkillSystem.getWeaponModifiers(state, loadout, weaponType)
      : { damageMultiplier: 1, armorPierceBonus: 0 };
    var automatic = scope.extensionWeaponSystem && scope.extensionWeaponSystem.getWeaponModifiers
      ? scope.extensionWeaponSystem.getWeaponModifiers(state, weaponType)
      : { damageMultiplier: 1, armorPierceBonus: 0 };
    function clamp01(value) { return Math.max(0, Math.min(1, Number(value) || 0)); }
    return {
      damageMultiplier: Math.max(0, Number(active.damageMultiplier) || 1) * Math.max(0, Number(automatic.damageMultiplier) || 1),
      armorPierceBonus: Math.min(1, Math.max(0, Number(active.armorPierceBonus) || 0) + Math.max(0, Number(automatic.armorPierceBonus) || 0)),
      normalBulletCancelRate: clamp01(Math.max(Number(active.normalBulletCancelRate) || 0, Number(automatic.normalBulletCancelRate) || 0)),
      eliteBulletCancelRate: clamp01(Math.max(Number(active.eliteBulletCancelRate) || 0, Number(automatic.eliteBulletCancelRate) || 0))
    };
  }
  ...
  function updateDecisiveCommand(state, dt) {
    ...
    if (runtime.rechargeTimer <= 0) {
      ...
      if (scope.bus && scope.events && scope.events.DECISIVE_COMMAND_READY) {
        scope.bus.emit(scope.events.DECISIVE_COMMAND_READY, { charges: runtime.charges, maxCharges: runtime.maxCharges });
      }
    }
  }
  ...
  function tryUseDecisiveCommand(state, loadout) {
    ...
    if (scope.bus && scope.events && scope.events.DECISIVE_COMMAND_USED) {
      var usedEffect = command.effect || {};
      scope.bus.emit(scope.events.DECISIVE_COMMAND_USED, {
        effectId: usedEffect.id || command.id,
        effectName: usedEffect.name || command.name
      });
    }
    return true;
  }
  ...
  function updateVisualEffects(state, dt) {
    ...
    var write = 0;
    for (var read = 0; read < effects.length; read += 1) {
      if (effects[read] && effects[read].life > 0) effects[write++] = effects[read];
    }
    effects.length = write;
    if (state) state.skillEffects = effects;
  }
