(function registerPhaseShield(root) {
  ...
  function activate(context) {
    ...
    var effect = {
      ...
      visualId: context.source === "auto" ? "auto-phase-shield" : "",
  ...
  function drawEffect(ctx, effect, alpha, progress) {
    ...
    var automatic = effect.visualId === "auto-phase-shield";
    ctx.strokeStyle = automatic ? "rgba(99,255,180," + (0.9 * alpha).toFixed(3) + ")" : "rgba(86,242,255," + (0.88 * alpha).toFixed(3) + ")";
    ctx.fillStyle = automatic ? "rgba(53,232,148," + (0.11 * alpha).toFixed(3) + ")" : "rgba(40,171,255," + (0.1 * alpha).toFixed(3) + ")";
    ctx.shadowColor = effect.color || (automatic ? "#63ffb4" : "#56f2ff");
  ...
  function getVisual(context) {
    var visual = context.source === "auto" && scope.skillVisualTheme && scope.skillVisualTheme.getAutoVisual
      ? scope.skillVisualTheme.getAutoVisual("auto-phase-shield")
      : null;
    return visual || { primary: "#56f2ff", secondary: "#d8ffff" };
  }
