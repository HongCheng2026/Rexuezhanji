(function registerBattleRuntime(root) {
  ...
  function startLevelBattle(level, profile, cloud, callbacks) {
    var fighterApi = scope.fighterUpgradeApi || scope.combatStats || {};
    var loadout = fighterApi.generateBattleLoadout
      ? fighterApi.generateBattleLoadout(profile)
      : null;
  ...
  function update(battleContext, dt) {
    ...
    if (scope.extensionWeaponSystem && scope.extensionWeaponSystem.update) {
      scope.extensionWeaponSystem.update(state, battleContext.loadout, dt);
    }
