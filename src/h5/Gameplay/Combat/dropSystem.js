(function registerDropSystem(root) {
  ...
  function allWeaponsMaxed(state) {
    return WEAPON_TYPES.every(function (type) {
      if (isWeaponDisabled(state, type)) return true;
  ...
  function getAvailableWeaponTypes(state) {
    var types = [];
    for (var i = 0; i < WEAPON_TYPES.length; i++) {
      if (!isWeaponDisabled(state, WEAPON_TYPES[i]) && getWeaponLevel(state, WEAPON_TYPES[i]) < MAX_WEAPON_LEVEL) types.push(WEAPON_TYPES[i]);
    }
    return types;
  }

  function isWeaponDisabled(state, type) {
    var disabled = state && state.player && state.player.disabledWeaponTypes;
    return Array.isArray(disabled) && disabled.indexOf(type) >= 0;
  }
  ...
  function updateCoins(state, dt, bountyLevel) {
    ...
    compactTimedList(state.coins, "x", -30);
  }
  ...
  function updatePowerups(state, dt) {
    ...
    compactTimedList(state.powerups, "x", -30);
  }
  ...
  function updateParticles(state, dt) {
    ...
    compactTimedList(state.particles, "life", 0);
  }
  ...
  function updateShockwaves(state, dt) {
    ...
    compactTimedList(state.shockwaves, "life", 0);
  }
  ...
  function updateNotices(state, dt) {
    ...
    compactTimedList(state.notices, "life", 0);
  }

  function compactTimedList(list, key, minimum) {
    var write = 0;
    for (var read = 0; read < list.length; read += 1) {
      var item = list[read];
      if (item && Number(item[key]) > minimum) list[write++] = item;
    }
    list.length = write;
    return list;
  }
  ...
  function applyPowerup(state, loadout, type) {
    ...
    if (isWeaponDisabled(state, type)) return;
    ...
    if (scope.bus && typeof scope.bus.emit === "function") {
      scope.bus.emit(events.WEAPON_UPGRADED, { type: type, level: state.player.weapons[type] });
    }
