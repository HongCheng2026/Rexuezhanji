(function registerSettlementSystem(root) {
  ...
  var progressionSystem = scope.progressionSystem || {};
  ...
  function completeLevelSetup(state, level, profile, cloudTicket) {
    ...
    if (scope.bus && typeof scope.bus.emit === "function") {
      scope.bus.emit(events.LEVEL_CLEARED, {
        level: level, coins: result.coinsEarned, exp: result.expEarned, rating: result.rating
      });
    }
    var settlement = {
    ...
    if (progressionSystem.completeLevel) {
      updatedProfile = progressionSystem.completeLevel(profile, level, result.rating);
    } else if (battleRules.completeLevel) {
