(function registerWeaponSystem(root) {
  ...
  function applyBulletSourceMetadata(bullet, spec) {
    if (!bullet || !spec) return bullet;
    if (spec.extensionWeaponId) bullet.extensionWeaponId = String(spec.extensionWeaponId);
    if (spec.normalBulletCancelRate != null) bullet.normalBulletCancelRate = Math.max(0, Math.min(1, Number(spec.normalBulletCancelRate) || 0));
    if (spec.eliteBulletCancelRate != null) bullet.eliteBulletCancelRate = Math.max(0, Math.min(1, Number(spec.eliteBulletCancelRate) || 0));
    return bullet;
  }

  function emitSpreadVolley(state, bullets, spec) {
    spec = spec || {};
    var count = Math.max(1, Math.floor(Number(spec.projectileCount) || 1));
    var baseAngle = Number(spec.angle) || 0;
    var step = Math.max(0, Number(spec.angleStep) || 0);
    var coverage = Math.max(0, Number(spec.coverageRadians) || 0);
    if (count > 1 && coverage > 0) step = coverage / (count - 1);
    var start = baseAngle - step * (count - 1) / 2;
    var originX = Number(spec.x) || 0;
    var originY = Number(spec.y) || 0;
    var mountOffset = Math.max(0, Number(spec.mountOffset) || 0);
    var created = [];
    for (var i = 0; i < count; i++) {
      var bullet = createBullet(
        originX,
        originY + (mountOffset ? (i % 2 === 0 ? -mountOffset : mountOffset) : 0),
        start + step * i,
        spec.type || "spread",
        Math.max(0, Number(spec.damage) || 0),
        Math.max(1, Number(spec.speed) || 610),
        Math.max(1, Number(spec.radius) || 4),
        spec.color || WEAPON_VISUALS.spread.color,
        {
          owner: "player",
          shape: spec.shape || "bolt",
          width: Math.max(1, Number(spec.width) || WEAPON_VISUALS.spread.width),
          height: Math.max(1, Number(spec.height) || WEAPON_VISUALS.spread.height),
          pierceRemaining: Math.max(0, Math.floor(Number(spec.pierceRemaining) || 0)),
          trailColor: spec.trailColor || WEAPON_VISUALS.spread.trailColor,
          armorPierceRatio: Math.max(0, Number(spec.armorPierceRatio) || 0),
          bulletVisualId: spec.bulletVisualId || ""
        }
      );
      applyBulletSourceMetadata(bullet, spec);
      bullets.push(bullet);
      created.push(bullet);
    }
    return created;
  ...
  function emitLaserVolley(state, bullets, spec) {
    spec = spec || {};
    var offsets = Array.isArray(spec.offsets) && spec.offsets.length ? spec.offsets : [0];
    var created = [];
    for (var i = 0; i < offsets.length; i++) {
      var bullet = createBullet(
        Number(spec.x) || 0,
        (Number(spec.y) || 0) + (Number(offsets[i]) || 0),
        Number(spec.angle) || 0,
        spec.type || "laser",
        Math.max(0, Number(spec.damage) || 0),
        Math.max(1, Number(spec.speed) || 920),
        Math.max(1, Number(spec.radius) || 5),
        spec.color || WEAPON_VISUALS.laser.color,
        {
          owner: "player",
          shape: spec.shape || "beam",
          width: Math.max(1, Number(spec.width) || WEAPON_VISUALS.laser.width),
          height: Math.max(1, Number(spec.height) || WEAPON_VISUALS.laser.height),
          pierceRemaining: Math.max(0, Math.floor(Number(spec.pierceRemaining) || 0)),
          trailColor: spec.trailColor || WEAPON_VISUALS.laser.trailColor,
          armorPierceRatio: Math.max(0, Number(spec.armorPierceRatio) || 0),
          bulletVisualId: spec.bulletVisualId || ""
        }
      );
  ...
  function emitMissileVolley(state, bullets, spec) {
    spec = spec || {};
    var count = Math.max(1, Math.floor(Number(spec.projectileCount) || 1));
    var offsets = createCenteredOffsets(count, Number(spec.offsetStep) || 14, Number(spec.maxAbsOffset) || 52);
    var locks = getMissileLocks(state);
    var targetLimit = Math.max(0, Math.floor(Number(spec.targetCount) || 0));
    var targetPool = targetLimit > 0 ? getPriorityMissileTargets(state, targetLimit, { x: Number(spec.x) || 0, y: Number(spec.y) || 0 }) : null;
    var speed = Math.max(1, Number(spec.speed) || 460);
    var created = [];
    for (var i = 0; i < offsets.length; i++) {
      var normalizedSlot = offsets.length === 1 ? 0 : (i / (offsets.length - 1)) * 2 - 1;
      var missile = createBullet(
        Number(spec.x) || 0,
        (Number(spec.y) || 0) + offsets[i],
        normalizedSlot * Math.max(0, Number(spec.angleSpread) || 0.22),
        "missile",
        Math.max(0, Number(spec.damage) || 0),
        speed,
        Math.max(1, Number(spec.radius) || 7),
        spec.color || WEAPON_VISUALS.missile.color,
        {
          owner: "player",
          shape: spec.shape || "lance",
          width: Math.max(1, Number(spec.width) || WEAPON_VISUALS.missile.width),
          height: Math.max(1, Number(spec.height) || WEAPON_VISUALS.missile.height),
          pierceRemaining: Math.max(0, Math.floor(Number(spec.pierceRemaining) || 0)),
          trailColor: spec.trailColor || WEAPON_VISUALS.missile.trailColor,
          splashRadius: Math.max(0, Number(spec.splashRadius) || 0),
          splashExcludesDirect: Number(spec.splashRadius) > 0,
          armorPierceRatio: Math.max(0, Number(spec.armorPierceRatio) || 0),
          bulletVisualId: spec.bulletVisualId || ""
        }
      );
      missile.homingStartAge = Math.max(0, Number(spec.homingStartAge) || 0.08);
      missile.homingTurnRate = Math.max(0.1, Number(spec.turnRate) || 6);
      missile.homingSteerGain = Math.max(0.1, Number(spec.steerGain) || 9);
      missile.launchSpeed = speed;
      missile.maxSpeed = speed * Math.max(1, Number(spec.maxSpeedMultiplier) || 1.3);
      missile.accelerationDuration = Math.max(0.01, Number(spec.accelerationDuration) || 0.35);
      var target = targetPool && targetPool.length
        ? targetPool[i % targetPool.length]
        : selectMissileTarget(state, missile, locks);
      missile.targetId = target && target.id ? target.id : "";
      if (missile.targetId) locks[missile.targetId] = (locks[missile.targetId] || 0) + 1;
      applyBulletSourceMetadata(missile, spec);
      bullets.push(missile);
      created.push(missile);
    }
    return created;
  }
  ...
  function shoot(state, loadout, bullets) {
    ...
    if (!activeWeapons.length) {
      if (state.player.suppressFallbackWeapon) return;
      var baseBullet = createBullet(
        x + 34, y, 0, "normal",
      ...
      applyBulletSourceMetadata(baseBullet, {});
  ...
  function fireWeapon(state, loadout, bullets, weapon, level, x, y) {
    ...
    var handler = FIXED_WEAPON_HANDLERS[weapon];
    ...
    var activeModifiers = scope.abilitySystem && scope.abilitySystem.getWeaponModifiers
      ...
      : { damageMultiplier: 1, armorPierceBonus: 0, normalBulletCancelRate: 0, eliteBulletCancelRate: 0 };
    ...
    var activeArmorPierce = baseArmorPierce
      + Math.max(0, Number(activeModifiers.armorPierceBonus) || 0);
    var baseDamage = Math.round(getPlayerDamage(loadout, weapon, level) * damageMultiplier);
    // 被动/自动技能赋予玩家子弹的敌弹抵消率；BOSS 子弹由 collisionSystem 自动豁免。
    var cancelRates = {
      normalBulletCancelRate: Math.max(0, Math.min(1, Number(activeModifiers.normalBulletCancelRate) || 0)),
      eliteBulletCancelRate: Math.max(0, Math.min(1, Number(activeModifiers.eliteBulletCancelRate) || 0))
    };
    return handler({
      state: state,
      loadout: loadout,
      ...
      activeArmorPierce: activeArmorPierce,
      baseDamage: baseDamage,
      cancelRates: cancelRates
    });
  }

  function fireSpreadWeapon(context) {
    ...
    return emitSpreadVolley(context.state, context.bullets, {
      x: context.x + 32,
      y: context.y,
      projectileCount: count,
      angleStep: step,
      damage: context.baseDamage,
      speed: 610 + context.level * 5,
      radius: 4,
      color: WEAPON_VISUALS.spread.color,
      width: WEAPON_VISUALS.spread.width,
      height: WEAPON_VISUALS.spread.height,
      pierceRemaining: getWeaponPierce(context.loadout, context.state, "spread"),
      trailColor: WEAPON_VISUALS.spread.trailColor,
      armorPierceRatio: context.activeArmorPierce,
      normalBulletCancelRate: context.cancelRates.normalBulletCancelRate,
      eliteBulletCancelRate: context.cancelRates.eliteBulletCancelRate
    }).length > 0;
  }

  function fireLaserWeapon(context) {
    var stats = context.stats;
    var effects = context.effects;
    var offsets = Array.isArray(stats.offsets) ? stats.offsets : [0];
    var laserPierce = getWeaponPierce(context.loadout, context.state, "laser") + Math.max(0, Math.floor(Number(effects.pierceBonus) || 0));
    var created = emitLaserVolley(context.state, context.bullets, {
      x: context.x + 38,
      y: context.y,
      offsets: offsets,
      damage: context.baseDamage,
      speed: 920 + context.level * 6,
      radius: 5,
      color: WEAPON_VISUALS.laser.color,
      width: 38 + Math.floor(context.level / 2),
      height: 5 + (context.level >= 8 ? 1 : 0),
      pierceRemaining: laserPierce,
      trailColor: WEAPON_VISUALS.laser.trailColor,
      armorPierceRatio: context.activeArmorPierce,
      normalBulletCancelRate: context.cancelRates.normalBulletCancelRate,
      eliteBulletCancelRate: context.cancelRates.eliteBulletCancelRate
    });
    if (Number(effects.triggerEvery) > 0) {
      context.state.player.weaponVolleyCounts = context.state.player.weaponVolleyCounts || {};
      var laserVolley = (context.state.player.weaponVolleyCounts.laser || 0) + 1;
      context.state.player.weaponVolleyCounts.laser = laserVolley;
      if (laserVolley % Math.floor(Number(effects.triggerEvery)) === 0) {
        emitLaserVolley(context.state, context.bullets, {
          x: context.x + 42,
          y: context.y,
          damage: Math.round(getPlayerDamage(context.loadout, "laser", context.level) * Math.max(1, Number(effects.bonusBeamDamageMultiplier) || 2) * Math.max(0, Number(context.activeModifiers.damageMultiplier) || 1)),
          speed: 980 + context.level * 6,
          radius: 7,
          color: "#bff8ff",
          width: 62,
          height: 9,
          pierceRemaining: laserPierce + 1,
          trailColor: "rgba(191,248,255,0.34)",
          armorPierceRatio: context.activeArmorPierce,
          normalBulletCancelRate: context.cancelRates.normalBulletCancelRate,
          eliteBulletCancelRate: context.cancelRates.eliteBulletCancelRate
        });
  ...
  function fireMissileWeapon(context) {
    var stats = context.stats;
    var effects = context.effects;
    var count = Math.max(1, Math.floor(Number(stats.projectileCount) || 1));
    count += Math.max(0, Math.floor(Number(effects.projectileBonus) || 0));
    if (Number(effects.projectileCap) > 0) count = Math.min(count, Math.floor(Number(effects.projectileCap)));
    return emitMissileVolley(context.state, context.bullets, {
      x: context.x + 28,
      y: context.y,
      projectileCount: count,
      damage: context.baseDamage,
      speed: (460 + context.level * 12) * Math.max(0.1, Number(effects.speedMultiplier) || 1),
      radius: 7,
      color: WEAPON_VISUALS.missile.color,
      width: 20,
      height: 10,
      pierceRemaining: getWeaponPierce(context.loadout, context.state, "missile"),
      trailColor: WEAPON_VISUALS.missile.trailColor,
      splashRadius: Math.max(0, Number(effects.splashRadius) || 0),
      armorPierceRatio: context.activeArmorPierce,
      normalBulletCancelRate: context.cancelRates.normalBulletCancelRate,
      eliteBulletCancelRate: context.cancelRates.eliteBulletCancelRate,
      turnRate: 6 * Math.max(0.1, Number(effects.turnMultiplier) || 1),
      steerGain: 9,
  ...
  var FIXED_WEAPON_HANDLERS = Object.freeze({
    spread: fireSpreadWeapon,
    laser: fireLaserWeapon,
  ...
  function playWeaponSfx(activeWeapons) {
    ...
    if (!activeWeapons || !activeWeapons.length) {
      ...
      for (var i = 0; i < activeWeapons.length; i++) {
        ...
        if (type === "laser") selectedType = "laser";
      }
    }
    if (scope.bus && typeof scope.bus.emit === "function") {
      scope.bus.emit(events.WEAPON_FIRED, { weaponType: selectedType, source: "primary" });
    }
  }
  ...
  function updateBullets(state, dt) {
    ...
    compactProjectiles(state.bullets, field);
  }
  ...
  function updateEnemyBullets(state, dt) {
    ...
    var field = getField(state);
    compactProjectiles(state.enemyBullets, field);
  ...
  function updateEnemyTelegraphs(state, dt) {
    ...
    for (var read = 0; read < list.length; read += 1) {
      if (list[read] && list[read].life > 0) list[write++] = list[read];
    }
    list.length = write;
    state.enemyTelegraphs = list;
  }

  function compactProjectiles(list, field) {
    var padding = Math.max(0, Number(field && field.cullPadding) || 0);
    var width = Math.max(0, Number(field && field.width) || 0);
    var height = Math.max(0, Number(field && field.height) || 0);
    var write = 0;
    for (var read = 0; read < list.length; read += 1) {
      var bullet = list[read];
      if (!bullet || bullet.dead || bullet.x <= -padding || bullet.x >= width + padding || bullet.y <= -padding || bullet.y >= height + padding) continue;
      list[write++] = bullet;
    }
    list.length = write;
    return list;
  ...
  function getPriorityMissileTargets(state, maxCount, origin) {
    var enemies = state && Array.isArray(state.enemies)
      ? state.enemies.filter(function validTarget(target) { return isValidMissileTarget(target, state); })
      : [];
    if (!enemies.length && isValidMissileTarget(state && state.boss, state)) enemies.push(state.boss);
    var point = origin || (state && state.player) || { x: 0, y: 0 };
  ...
  var api = {
    ...
    emitSpreadVolley: emitSpreadVolley,
    emitLaserVolley: emitLaserVolley,
    emitMissileVolley: emitMissileVolley,
    autoShoot: autoShoot,
    shoot: shoot,
    fireWeapon: fireWeapon,
    getFixedWeaponHandler: function getFixedWeaponHandler(weaponType) { return FIXED_WEAPON_HANDLERS[String(weaponType || "")] || null; },
