(function registerGachaView(root) {
  ...
  function create(options) {
    ...
    function resultCard(item, index) {
      ...
      var style = '--card-delay:' + (1150 + index * 90) + 'ms;' + (frame ? '--reward-frame:url(&quot;' + attr(frame) + '&quot;);' : '');
    ...
    function resultLayer(model) {
      ...
      var cards = model.results.map(resultCard).join("");
      ...
      return '<section class="gacha-result-layer' + (ultimate ? ' has-ultimate' : '') + '" aria-label="抽取结果">' +
        ...
        '<div class="gacha-result-box"><header><small>SIGNAL ACQUIRED</small><strong>' + (ultimate ? '终极信号确认' : '跃迁结果') + '</strong></header>' +
          ...
          '<footer class="gacha-result-actions"><button type="button" data-gacha-action="back">← 返回</button><button class="is-primary" type="button" data-gacha-action="redraw">再抽' + redrawCount + '次 <small>消耗 ' + config.DRAW_COSTS[redrawCount] + ' 券</small></button></footer>' +
        '</div></section>';
    }
    ...
    function topUpDialog(model) {
      ...
      if (!pending) return "";
      return '<section class="gacha-topup-layer" role="dialog" aria-modal="true" aria-label="购买研究券"><article><small>RESEARCH TICKET SUPPLY</small><h2>星链研究券不足</h2><p>本次需要 <strong>' + pending.ticketCost + '</strong> 张，当前还差 <strong>' + pending.missingTickets + '</strong> 张。是否花费 <strong>' + pending.diamondCost + ' 钻石</strong> 补足并继续抽取？</p><div><span>钻石余额 ' + format(model.diamonds) + '</span><span>' + (pending.canAfford ? '确认后一次性结算' : '钻石不足，无法补购') + '</span></div><footer><button type="button" data-gacha-action="cancel-topup">取消</button><button class="is-primary" type="button" data-gacha-action="confirm-topup"' + (pending.canAfford ? '' : ' disabled') + '>购买并抽取</button></footer></article></section>';
    }
    ...
    function render(model) {
      ...
      mount.innerHTML =
        '<article class="gacha-terminal"' + terminalStyle + '>' +
          ...
          '</div>' +
          '<footer class="gacha-actions"><p class="' + (model.isError ? 'is-error' : '') + '">' + escapeHtml(model.message || '研究券为唯一抽取货币；十连 9 折，只消耗 9 张。') + '</p><div><button type="button" data-gacha-action="draw" data-gacha-count="1"' + (canDraw ? '' : ' disabled') + '>单次跃迁 <span>×1</span> <small>1 券</small></button><button class="is-primary" type="button" data-gacha-action="draw" data-gacha-count="10"' + (canDraw ? '' : ' disabled') + '>十连跃迁 <span>×10</span> <small>9 券 · 9折</small></button></div></footer>' +
