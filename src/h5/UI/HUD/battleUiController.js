(function registerBattleUiController(root) {
  ...
  function createActiveSlotModels(runtimeSlots, configuredSlots) {
    ...
    for (var i = 0; i < 4; i += 1) {
      ...
      result.push(config && current ? {
        ...
        charges: current.charges == null ? null : Math.max(0, Math.floor(Number(current.charges) || 0)),
        maxCharges: current.maxCharges == null ? null : Math.max(1, Math.floor(Number(current.maxCharges) || 1)),
        rechargeRemaining: Math.max(0, Number(current.rechargeTimer) || 0),
  ...
  function getActiveSlotStatus(runtime) {
    ...
    if (runtime.charges != null && Number(runtime.charges) <= 0) return "charging";
  ...
  function createWeaponSkillModels(state, player) {
    ...
    var slots = Array.isArray(skills.slots) ? skills.slots : [];
    var fixedFallbacks = [
    ...
    var result = fixedFallbacks.map(function createFixedModel(fallback, index) {
      ...
      if (runtime && runtime.source === "meta") return createMetaModel(runtime);
    ...
    for (var i = 0; i < 3; i += 1) {
      var extension = slots[i + 3];
      if (!extension) {
        result.push(null);
        continue;
      }
      result.push(createMetaModel(extension));
    }
    return result;
  }

  function createMetaModel(runtime) {
    var glyphs = {
      weapon_module_04: "翼", weapon_module_05: "轨", weapon_module_06: "蜂",
      "passive-front-spread": "扇", "passive-railgun": "炮", "passive-shockwave": "波", "passive-chain-lightning": "电",
      "sky-lock-beam": "锁", "obsidian-gravity-well": "域", "gold-judgement-buff": "裁", "phase-shield": "相"
    };
    return {
      id: runtime.id,
      name: runtime.name,
      iconText: glyphs[runtime.id] || "技",
      source: "meta",
      level: Math.max(1, Math.floor(Number(runtime.level) || 1)),
      status: "auto"
    };
  }
