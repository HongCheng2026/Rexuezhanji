(function registerSkyLockBeam(root) {
  ...
  function fireNextBeam(context) {
    ...
    var damage = Math.round(referenceDamage * (Number(skill.damageBudget) || 1) / count);
    state.bullets.push(scope.weaponSystem.createBullet(originX, originY, angle, "skyLockBeam", damage, 1260, 7, "#a8f6ff", {
      owner: "player",
      shape: "beam",
      width: 88,
      height: 7,
      pierceRemaining: 999,
      trailColor: "rgba(130,247,255,0.3)"
    }));
    addEffect(state, {
      type: SKILL_ID,
  ...
  function drawEffect(ctx, effect, alpha, progress) {
    ...
    ctx.shadowColor = effect.color || "#a8f6ff";
