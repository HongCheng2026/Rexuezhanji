...
export function createEconomyService(deps: Dependencies) {
  ...
  function applyRewards(profile: any, rewards: Reward[]) {
    ...
    for (const reward of rewards) {
      ...
      if (reward.type === "energy") profile.resources.energy = Math.min(profile.resources.maxEnergy, Math.max(0, Math.floor(Number(profile.resources.energy) || 0) + amount));
  ...
  function getTaskMetric(taskId: string, profile: any) {
    ...
    const metrics: Record<string, number> = {
      ...
      task_rank_ship: ["A", "S"].includes(deps.shipRankById[String(profile.scene?.shipId || "")] || "B") ? 1 : 0
    };
